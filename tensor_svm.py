...
def train_svm(train_set, epoch_size = 1, c = 50.0, gamma = .01):
...
def main(args):
    if len(args)<=1:
        ...
        whole_set = np.loadtxt("TF_features/10Categories.csv", delimiter=",")
        np.random.shuffle(whole_set)
        num_points = whole_set.shape[0]
        train_set, test_set = whole_set[:num_points*9/10,:], whole_set[num_points*9/10:,:]
        ...
        for lab in label:
            ...
            for idx in range(len(new_train_set)):
                if new_train_set[idx,-1] != lab:
                    new_train_set[idx,-1] = -1
